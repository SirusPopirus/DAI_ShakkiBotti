...
void Sotilas::annaSiirrot(std::list<Siirto>& lista, Ruutu* ruutu, Asema* asema, int vari)
{
    ...
    // ohestalyönti
    int enPassantRivi = (vari == 0 ? 4 : 3);
    if (r == enPassantRivi && asema->kaksoisaskelSarakkeella != -1) {
        int targetFile = asema->kaksoisaskelSarakkeella;
        if (abs(targetFile - s) == 1) {
            int lr = r + suunta;
            int ls = targetFile;
            if (lr >= 0 && lr < 8) {
                lista.push_back(Siirto(Ruutu(r, s), Ruutu(lr, ls)));
            }
        }
    }
}
